...
//  Romio
//
//  Created by Arjun Busani on 4/5/17.
...
enum NavigationBarAnimationDirection {
    case right
    case left
}
...
class CustomNavBarViewController: UIViewController {
    ...
    let safeAreaStatusView: UIView = UIView()
    let seperator = UIView()

    var navigationRect: CGRect {
        get {
            let statusBarHeight = UIApplication.shared.statusBarFrame.height
            return CGRect(x: 0, y: self.navBarHeight, width: UIScreen.size.width, height: UIScreen.size.height - self.navBarHeight - statusBarHeight)
        }
    }

    var interstitialNavBar = InterstitialNavigationBar()
    var interstitialRect: CGRect {
        return CGRect(x: 0, y: statusBarHeight, width: UIScreen.size.width, height: InterstitialNavigationBar.viewHeight())
    }
    let interstitialNavHeight: CGFloat = InterstitialNavigationBar.viewHeight()
    ...
    var navBarHeight: CGFloat {
        get {
            let topOffset = UIScreen.isPhoneX ? statusBarHeight : 0
            return seperatorHeight + customNavBarHeight + topOffset
        }
    }
...
//    var messageView: RomioMessage?
    ...
    internal func setupNavBar() {
        ...
        safeAreaStatusView.frame = CGRect(x: 0, y: 0, width: view.frame.size.width, height: topOffset)
        safeAreaStatusView.backgroundColor = .white
        view.addSubview(safeAreaStatusView)

        navigationBar.frame = CGRect(x: 0, y: topOffset, width: view.frame.size.width, height: customNavBarHeight)
        view.addSubview(navigationBar)
        
        seperator.backgroundColor = UIColor.romioLightGray
        seperator.frame = CGRect(x: 0, y: navigationBar.frame.height + topOffset, width: view.frame.width, height: seperatorHeight)
        view.addSubview(seperator)
    }
    
    func setupInterstitialNavBar() {
        interstitialNavBar.removeFromSuperview()
        interstitialNavBar.frame = interstitialRect
        view.addSubview(interstitialNavBar)
    }
    ...
    func setNavBarTitle(ofType type: NavigationBarTitleType = .title, position: NavigationElementPosition = .center, customView: UIView? = nil, title: String = "", titleColor: UIColor = .black, fontSize: FontSize = .largeMedium, font: UIFont = UIFont.sfDisplayOfStyleAndSize(style: .bold, size: .largeMedium), navBarColor: UIColor = UIColor.white, includeSingleBarButton: Bool = false, isTransparent: Bool = false, leadingOffset: CGFloat = UIView.narrowMidMargin, maxNumberOfLines: Int = 1) -> UISearchBar {
        safeAreaStatusView.backgroundColor = navBarColor
        return navigationBar.setNavBarTitle(ofType: type, position: position, customView: customView, title: title, titleColor: titleColor, fontSize: fontSize, font: font, navBarColor: navBarColor, includeSingleBarButton: includeSingleBarButton, isTransparent: isTransparent, leadingOffset: leadingOffset, maxNumberOfLines: maxNumberOfLines)
    }
    
    func addGradientLayer(with colors: [UIColor], point: GradientPoint = .topBottom(offset: CGFloat.topBottomGradiantXOffset)) {
        safeAreaStatusView.backgroundColor = .clear
        navigationBar.addGradientLayer(with: colors, point: point)
    }
    
    func hideInterstitialNavBar() {
        interstitialNavBar.removeFromSuperview()
    }
...
//    }
    ...
    internal func setupProgressView() {
        ...
        progressView.snp.remakeConstraints { (make) in
            ...
            make.top.equalTo(view).offset(navBarHeight+(NavigationProgressView.lineHeight/2))
