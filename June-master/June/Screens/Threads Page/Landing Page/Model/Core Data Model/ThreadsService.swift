...
//  June
//
//  Created by Joshua Cleetus on 8/25/17.
...
class ThreadsService {
    ...
    internal init(parentVC: ThreadsViewController) {
    ...
    func update(currentThreadStarredValue currentThread: Threads, withNewStarredValue newStarred: Bool, withNewUnreadValue newUnread: Bool) {
        currentThread.unread = newUnread
    ...
    // Unread
    func getAllUnreadConversations(withAscendingOrder ascending: Bool) -> NSFetchedResultsController<Threads> {
    ...
    //ReadAndSent
    func getAllReadConversations(withAscendingOrder ascending: Bool) -> NSFetchedResultsController<Threads> {
        let request: NSFetchRequest<Threads> = Threads.fetchRequest()
        var sortDescriptors = [NSSortDescriptor]()
        let sort = NSSortDescriptor(key: "last_message_timestamp", ascending: ascending)
        sortDescriptors.append(sort)
...
//        self.parentVC.readTableView?.reloadData()
    }
    ...
    func getTrashConversations(withAscendingOrder ascending: Bool) -> NSFetchedResultsController<Threads> {
        let request: NSFetchRequest<Threads> = Threads.fetchRequest()
        var sortDescriptors = [NSSortDescriptor]()
        let sort = NSSortDescriptor(key: "last_message_received_timestamp", ascending: ascending)
        sortDescriptors.append(sort)
        request.sortDescriptors = sortDescriptors
        request.fetchBatchSize = 20
        ...
        let fetchedResultsController = NSFetchedResultsController(fetchRequest: request, managedObjectContext: CoreDataManager.sharedInstance.persistentContainer.viewContext, sectionNameKeyPath: nil, cacheName: nil)
        self.parentVC.fetchedResultController3 = fetchedResultsController
        do {
            try fetchedResultsController.performFetch()
        }
        catch let error as NSError {
    ...
    func saveInCoreDataWith3(array: [[String: AnyObject]]) {
        ...
        self.parentVC.trashTableView?.reloadData()
    }
    ...
    func getSpamConversations(withAscendingOrder ascending: Bool) -> NSFetchedResultsController<Threads> {
        ...
        request.predicate = predicate1
        let fetchedResultsController = NSFetchedResultsController(fetchRequest: request, managedObjectContext: CoreDataManager.sharedInstance.persistentContainer.viewContext, sectionNameKeyPath: nil, cacheName: nil)
        self.parentVC.fetchedResultController4 = fetchedResultsController
        do {
            try fetchedResultsController.performFetch()
        }
        catch let error as NSError {
    ...
    func saveInCoreDataWith4(array: [[String: AnyObject]]) {
        _ = array.map{self.createThreadsEntityFrom(dictionary: $0)}
        CoreDataManager.sharedInstance.persistentContainer.viewContext.perform({
            do {
                try CoreDataManager.sharedInstance.persistentContainer.viewContext.save()
        ...
        self.parentVC.loadData4()
        self.parentVC.spamTableView?.reloadData()
    }
