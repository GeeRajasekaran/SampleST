...
protocol ConvosNewPreviewCellDelegate: class {
...
class ConvosNewPreviewTableViewCell: UITableViewCell, ConvosNewPreviewDelegate {
    
    let mailView: ConvosNewPreviewView = ConvosNewPreviewView(frame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.size.width, height: ConvosNewView.heightForView()))
    var bottomSeperatorView: UIView?
    weak var mDelegate: ConvosNewPreviewCellDelegate?
    ...
    class func reuseIdentifier() -> String {
        return "ConvosNewPreviewTableViewCell"
    }
    
    class func heightForCell(items: CGFloat = 3) -> CGFloat {
        return ConvosNewPreviewView.fixedHeight() * items
    }
    
    func convosNewPreview(didTapItem sender: ConvosNewPreviewView) {
        self.clipsToBounds = true
        if let mDelegate = self.mDelegate, let indexPath = self.indexPath {
            mDelegate.convosNewPreview(didTapItem: self, thread: sender.thread, indexPath: indexPath)
        }
    }
        ...
        UIView.animate(withDuration: 0.05, animations: {
            ...
            if let strongSelf = self {
                if let delegate = strongSelf.mDelegate, let thread = strongSelf.thread, let indexPath = strongSelf.indexPath {
                    ...
                    delegate.convosNewPreview(didSwipeItem: strongSelf, thread: thread, indexPath: indexPath, xValue: strongSelf.frame.origin.x)
                }
            }
        }
    }
