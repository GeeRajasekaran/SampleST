...
protocol ConvosNewPreviewDelegate: class {
    func convosNewPreview(didTapItem sender: ConvosNewPreviewView)
}

class ConvosNewPreviewView: UIView {
    ...
    weak var delegate: ConvosNewPreviewDelegate?
    ...
    func setupView() {
        ...
        fromLabel.font = UIFont.latoStyleAndSize(style: .black, size: .largeMedium)
        ...
        subjectLabel.font = UIFont.latoStyleAndSize(style: .bold, size: .medium)
        ...
        clearButton.setImage(#imageLiteral(resourceName: "clear-circle-checked"), for: .highlighted)
        clearButton.addTarget(self, action: #selector(ConvosNewPreviewView.clearButtonPressed(_:)), for: .touchUpInside)
        addSubview(clearButton)
        
        clearButtonOverlay.sizeToFit()
        clearButtonOverlay.addTarget(self, action: #selector(ConvosNewPreviewView.clearButtonPressed(_:)), for: .touchUpInside)
    ...
    @objc func clearButtonPressed(_ sender: UIButton) {
        ...
        if let delegate = self.delegate {
            delegate.convosNewPreview(didTapItem: self)
        }
    }
