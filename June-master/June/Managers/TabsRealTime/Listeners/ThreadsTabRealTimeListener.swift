//
//  ThreadsTabRealTimeListener.swift
//  June
//
...
class ThreadsTabRealTimeListener: IRealTimeListener {
...
extension ThreadsTabRealTimeListener: IRealTimeValidateable {
    ...
    func validate(_ object: [String : Any]) -> Bool {
        guard let approved = object[ValidationField.approved] as? Bool,
            ...
            let threadTimestamp = TimestampUserDefaults().threadsTimestamp else { return false }

        return (approved && category == ValidationThreshold.category && unread && lastMessageReceivedTime > threadTimestamp) == true
    }
}
